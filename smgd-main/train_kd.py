...
parser.add_argument('--arch', type=str, default='resnet50', help='the network')    # resnet152/densenet201
...
def save_checkpoint(state, is_best, save_root):
	save_path = os.path.join(save_root,str(args.arch)+'/'+str(args.T)+'/checkpoint.pth.tar')
...
def train(train_loader, nets, optimizer, criterions, epoch):
    ...
    for i, (img, target, idx) in enumerate(train_loader):
