...
def run_attack(attack, use_Inc_model=False):
    ...
    for true_label in tqdm(image_dict.keys()):
        ...
        if attack is ILA:
            ifgsm_guide = ifgsm(model, image_batch, label_batch, learning_rate=2. / 255., epsilon=opt.epsilon,
                                niters=10, dataset='imagenet')
            # fgsm_guide = fgsm(model, image_batch, label_batch, epsilon=opt.epsilon, dataset='imagenet')
            adversaries = ILA(model, image_batch, ifgsm_guide, label_batch, source_layers[opt.layerindex][1][1],
                              learning_rate=opt.learning_rate, epsilon=opt.epsilon, niters=opt.niters,
                              dataset='imagenet', use_Inc_model=use_Inc_model)
        elif attack is FIA:
            adversaries = FIA(model, image_batch, label_batch, source_layers[opt.layerindex][1][1],
                              learning_rate=opt.learning_rate, epsilon=opt.epsilon, niters=opt.niters,
                              dataset='imagenet', use_Inc_model=use_Inc_model)
        elif attack is AA:
            ...
            adversaries = AA(model, image_batch, label_batch, target_batch, source_layers[opt.layerindex][1][1],
                             learning_rate=opt.learning_rate, epsilon=opt.epsilon, niters=opt.niters,
                             dataset='imagenet', use_Inc_model=use_Inc_model)
        elif attack is Transferable_Adversarial_Perturbations:
            adversaries = Transferable_Adversarial_Perturbations(model, image_batch, label_batch,
                                                                 learning_rate=opt.learning_rate, epsilon=opt.epsilon,
                                                                 niters=opt.niters, dataset='imagenet',
                                                                 use_Inc_model=use_Inc_model)
        elif attack is NAA:
            adversaries = NAA(model, image_batch, label_batch, source_layers[opt.layerindex][1][1],
                              learning_rate=opt.learning_rate, epsilon=opt.epsilon, niters=opt.niters,
                              dataset='imagenet', use_Inc_model=use_Inc_model)
...
if __name__ == '__main__':
    ...
    opt = Namespace(modeltype='ResNet50', layerindex=2, filename='ResNet50_ILA_16_50_2_2_ifgsm', niters=50,
                    epsilon=16. / 255., learning_rate=2. / 255.)
