...
parser.add_argument('--output_dir', default='./output/densenet201/BIM/diffusion18_t_', help='the path of the saved dataset')
parser.add_argument('--attack_method', default='pgd', type=str,choices=['fgsm','pgd'])
parser.add_argument('--batch_size', type=int, default=25, metavar='N',help='input batch size for adversarial attack')
parser.add_argument('--ensemble', default=0, type=int) # 1为True
parser.add_argument('--momentum', default=0.0, type=float) # 1.0,就是MI; 0.0,就是不使用MI
...
parser.add_argument("--amplification", type=float, default=0.0, help="To amplifythe step size.") ## PI,10;+SMGD,5;加所有组合,2.5 ,gamma再乘0.5;不使用就是0.0
parser.add_argument("--pi_size", type=int, default=3, help="k size")

parser.add_argument('--ensemble_num', default='0', type=int) ### 集成网络的个数，到底集成几个学生网络 每次实验前都检查学生网路权重
parser.add_argument('--arch', default='densenet201',help='source model for black-box attack evaluation',choices=model_names)

parser.add_argument('--snet_dir1', default='./result/densenet201/18/1gpu_checkpoint.pth.tar', help='the path of snet1')
parser.add_argument('--snet_dir2', default='./result/densenet201/20/1gpu_checkpoint.pth.tar', help='the path of snet2')
parser.add_argument('--snet_dir3', default='./result/densenet201/22/1gpu_checkpoint.pth.tar', help='the path of snet3')
parser.add_argument('--snet_dir4', default='./result/densenet201/24/1gpu_checkpoint.pth.tar', help='the path of snet4')
parser.add_argument('--snet_dir5', default='./result/densenet201/26/1gpu_checkpoint.pth.tar', help='the path of snet5')
parser.add_argument('--snet_dir6', default='./result/densenet201/28/1gpu_checkpoint.pth.tar', help='the path of snet6')
parser.add_argument('--snet_dir7', default='./result/densenet201/30/1gpu_checkpoint.pth.tar', help='the path of snet7')
parser.add_argument('--snet_dir8', default='./result/densenet201/32/1gpu_checkpoint.pth.tar', help='the path of snet8')
# parser.add_argument('--snet_dir9', default='./result/densenet201/21/1gpu_checkpoint.pth.tar', help='the path of snet9')
# parser.add_argument('--snet_dir10', default='./result/densenet201/23/1gpu_checkpoint.pth.tar', help='the path of snet10')
####################
parser.add_argument('--cuda', type=int, default=[0,1])
...
parser.add_argument('--step_size', default=-1, type=float, help='perturb step size') # pgd 2; bim -1
...
# def load_checkpoint(model, pre_dict):
#     model_dict = model.state_dict()
#     new_dict = {}
#     for k,v in pre_dict.items():
#         k1 = k[7:]
#         new_dict[k1] = v
#     new1_dict = {k:v for k,v in new_dict.items() if k in model_dict }
#     model_dict.update(new1_dict)
...
def main():
    ...
    # tmodel = torch.nn.DataParallel(tmodel, list(range(2))).cuda()

    ### 进行diffusion和ensemble的对比实验，需要只加载蒸馏出的学生网络权重
    ...
    smodel1 = torch.nn.DataParallel(smodel1, list(range(2))).cuda()
    tmodel = smodel1
    ...
    if args.ensemble == True:
        ...
        smodel2 = torch.nn.DataParallel(smodel2, list(range(2))).cuda()
        # smodel2.cuda()
        smodel2.eval()
        # ### snet3
        ...
        smodel3 = torch.nn.DataParallel(smodel3, list(range(2))).cuda()
        smodel3.eval()
        # ### snet4
        ...
        smodel4 = torch.nn.DataParallel(smodel4, list(range(2))).cuda()
        smodel4.eval()
        # ### snet5
        ...
        smodel5 = torch.nn.DataParallel(smodel5, list(range(2))).cuda()
        smodel5.eval()
         ### snet6
        ...
        smodel6 = torch.nn.DataParallel(smodel6, list(range(2))).cuda()
        smodel6.eval()
        # # # ### snet7
        ...
        smodel7 = torch.nn.DataParallel(smodel7, list(range(2))).cuda()
        smodel7.eval()
        # # ### snet8
        ...
        smodel8 = torch.nn.DataParallel(smodel8, list(range(2))).cuda()
        smodel8.eval()
        # # # ### snet9
        # snet9= models.__dict__[args.arch](pretrained=False)
        # smodel9 = nn.Sequential(Normalize(mean=mean, std=std), snet9)
        # checkpoint9 = torch.load(args.snet_dir9)  # initial parameters of student model
        # load_pretrained_model(smodel9, checkpoint9['snet'])
        # smodel9 = torch.nn.DataParallel(smodel9, list(range(3))).cuda()
        # smodel9.eval()
        # ### snet10
        # snet10 = models.__dict__[args.arch](pretrained=False)
        # smodel10 = nn.Sequential(Normalize(mean=mean, std=std), snet10)
        # checkpoint10 = torch.load(args.snet_dir10)  # initial parameters of student model
        # load_pretrained_model(smodel10, checkpoint10['snet'])
        # smodel10 = torch.nn.DataParallel(smodel10, list(range(3))).cuda()
        # smodel10.eval()
    ...
    epsilon = args.epsilon / 255.0 # 16/255
    if args.step_size < 0:
        step_size = epsilon / args.num_steps  # （16/255）/10
        print("<0")
    else:
        step_size = args.step_size / 255.0  # 2/255
